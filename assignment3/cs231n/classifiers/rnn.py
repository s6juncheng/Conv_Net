...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    ############################################################################
    h0 = features.dot(W_proj) + b_proj #  (N,D) X (D,H) = (N,H)
    wordvec, embed_cache = word_embedding_forward(captions_in, W_embed) # (N, T, W)
  ...
  def sample(self, features, max_length=30):
    ...
    for i in xrange(max_length):
        ...
        if self.cell_type == "rnn":
            next_h, _ = rnn_step_forward(x, prev_h, prev_c, Wx, Wh, b)
        else:
